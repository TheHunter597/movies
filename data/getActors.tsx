...
function getActorsInfoOptions(entry:string){
    return {
        params: {id: entry},
        headers: {
          'X-RapidAPI-Key': "b1dd9da26emsh638800ec2563a15p142a95jsnf0dae955870c",
...
export function getActorsInfo(type:"movie",id:string){
    let data = fetchData(`https://movie-details1.p.rapidapi.com/imdb_api/${type}`,getActorsInfoOptions(id))
    return data
}
...
export function getActorImage(entry:string){
    let data = fetchData("https://google-image-search1.p.rapidapi.com/v2/",getActorImageOptions(entry))
    return data
}